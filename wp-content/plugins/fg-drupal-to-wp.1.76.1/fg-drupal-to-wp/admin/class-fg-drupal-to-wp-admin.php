...
if ( !class_exists('FG_Drupal_to_WordPress_Admin', false) ) {
	...
	class FG_Drupal_to_WordPress_Admin extends WP_Importer {
		...
		public function dispatch($action) {
			...
			elseif ( !empty($action) ) {
				switch($action) {
					...
					// Delete content
					case 'empty':
						if ( check_admin_referer( 'empty', 'fgd2wp_nonce_empty' ) ) { // Security check
					...
					case 'modify_links':
						if ( check_admin_referer( 'modify_links', 'fgd2wp_nonce_modify_links' ) ) { // Security check
		...
		private function display_admin_page() {
			...
			$data['description'] = __('This plugin will import articles, stories, pages, images, categories and tags from a Drupal database into WordPress.<br />Compatible with Drupal versions 5, 6, 7 and 8.', 'fg-drupal-to-wp');
...
SQL;
			...
			do {
				...
				if ( is_array($nodes) ) {
					foreach ( $nodes as $node ) {
						if ( $this->import_node($node, $content_type, $entity_type) === false ) {
							return false;
						}
		 ...
		 */
		private function import_node($node, $content_type, $entity_type='node') {
		 ...
		 */
		public function import_media($name, $filename, $date, $attributes=array(), $options=array()) {
			...
			if ( preg_match('/^http/', $filename) ) {
				...
				if ( strpos($filename, '/') === 0 ) { // Absolute path
					...
					$old_filename = trailingslashit($domain) . $filename;
				} else {
